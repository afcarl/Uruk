...
parser.add_argument('--logDataFile', type=str, default='./logs/penn_log.txt',
...
parser.add_argument('--save', type=str,  default='./models/model_penn.pt',
                    help='path to save the final model')
args = parser.parse_args()

logFile = open(args.logDataFile,'w')
...
logFile.write("Batch size - " + str(args.bptt)+ "\n")
...
if torch.cuda.is_available():
    if not args.cuda:
        logFile.write("WARNING: You have a CUDA device, so you should probably run with --cuda")
...
def train():
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        if batch == 0 and epoch == 1:
            ...
            logFile.write('| epoch {:3d} | {:5d}/{:5d} batches | lr {:02.2f} | '
                  'loss {:5.2f} | ppl {:8.2f} |\n'.format(
...
logFile.write("Program ended at: "+ str(time.time()) + "\n")
logFile.write("Run took: {:5.2f}s\n".format(time.time() - prog_started_time))
print("Program ended at: ", time.time())
